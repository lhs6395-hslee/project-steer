...
하네스 파이프라인은 7개 업무 모듈(pptx, docx, wbs, trello, dooray, google_workspace, datadog)의 산출물을 자동 생성·검증하는 멀티 에이전트 시스템이다. GAN 영감의 Generator-Evaluator(Executor-Reviewer) 패턴을 적용하여, 산출물 생성과 검증을 독립 에이전트로 분리하고 적대적 리뷰 루프를 통해 품질을 보장한다.
...
│   ├── google_workspace/SKILL.md              # R29: 파일 관리
...
**내부 흐름:**
1. 모듈명 유효성 검증 (`pptx|docx|wbs|trello|dooray|google_workspace|datadog`)
...
{
  "task": "string — 작업 설명",
  "module": "enum — pptx|docx|wbs|trello|dooray|google_workspace|datadog",
  "steps": [
    {
      "id": "integer — 단계 번호",
