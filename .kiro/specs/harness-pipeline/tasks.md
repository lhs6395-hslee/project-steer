...
- [x] 1. 테스트 인프라 구축 및 Guardian 속성 검증
  - [x] 1.1 테스트 디렉토리 구조 및 Hypothesis 설정
  ...
  - [x] 1.2 Guardian 차단 로직 개선 — 누락 패턴 보강
...
- [x] 2. Confidence_Trigger 점수 계산 및 모드 매핑 검증
  - [x] 2.1 Confidence_Trigger 보안 키워드 강제 로직 개선
...
- [x] 4. JSON 스키마 검증 및 Verdict 비즈니스 규칙
  - [x] 4.1 JSON 스키마 round-trip 검증 유틸리티 작성
  ...
  - [x] 4.3 Verdict 비즈니스 규칙 검증 로직 추가
...
- [x] 5. IDE_Adapter 및 Atomic_Write 검증
  - [x] 5.1 IDE_Adapter 기본값 경고 메시지 추가
...
- [x] 7. Checkpoint — 핵심 속성 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. MCP Toggle 동기화 및 KAIROS 크리덴셜 감지
  - [x] 8.1 MCP Toggle 동기화 상태 검증 로직 추가
...
- [x] 9. 토큰 예산 관리 및 에이전트 실행 체계 보강
  - [x] 9.1 토큰 예산 설정 파일 및 예산 초과 로직 구현
...
- [x] 10. SDD 통합 및 Agent_Team 개선
  - [x] 10.1 SDD_Integrator 파일 경로 수정
...
- [x] 11. Orchestrator Handoff_File 생성 및 파이프라인 완성
  - [x] 11.1 Orchestrator 요청 Handoff_File 생성 추가
...
- [x] 12. Smoke Test 및 크로스 플랫폼 검증
  - [x] 12.1 Smoke Test 스크립트 작성
