...
Executor는 실행 전에 반드시 제약 조건을 확인한다:
1. Sprint Contract의 `constraints` 필드와 Module SKILL 디자인 규칙을 모두 수집
2. 각 실행 액션이 제약을 위반하지 않는지 사전 검증
3. 위반 시 대안을 찾거나 `blocked_by_constraint`로 보고
4. 출력에 `constraint_compliance` 필드 포함 (각 제약별 PASS/FAIL)
5. 재시도 시 `retry_fixes` 필드로 이전 이슈별 수정 내역 추적

**제약 조건 우선순위**: Module SKILL > Sprint Contract constraints > acceptance criteria > 자체 판단

## Reviewer v2.0: Constraint Verification

Reviewer는 가장 먼저 제약 조건 준수를 검증한다:
1. Executor의 `constraint_compliance` 필드를 독립 검증 (없으면 FAIL)
2. 재시도 시 `retry_fixes` 필드로 이전 피드백 반영 여부 확인 (없으면 FAIL)
3. 출력에 `constraint_violations`, `retry_fix_assessment` 필드 포함
4. 제약 위반 시 score 0.3 상한
