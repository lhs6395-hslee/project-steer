...
34. **L01/L02 다이어그램 영역을 텍스트만 교체하여 재사용** — L01 좌 패널 하단 미니플로우(노드1→노드2→노드3)와 L02 가운데 아키텍처 다이어그램은 특정 주제(예: MSK)에 맞게 설계된 주제 종속 콘텐츠다. 다른 주제의 발표자료를 만들 때 노드 텍스트만 교체하면 시각 구조와 내용이 불일치하는 슬라이드가 만들어진다.
   - **판단 기준:** 구조(노드 수·방향·계층)가 동일하면 텍스트 교체 허용. 구조가 다르면 해당 다이어그램 영역 전체를 새로 설계 필수.
   - **재발방지:** 새 발표자료 조립 시 L01/L02 다이어그램 영역에 대해 "이 주제의 구조가 템플릿과 동일한가?" 를 먼저 확인. layout-spec.md L01/L02 섹션의 ⚠️ 재사용 규칙 참조.
