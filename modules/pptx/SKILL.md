...
- 아이콘 크기: 411,480 × 411,480 EMU (0.45" × 0.45"), 카드 우상단 배치
...
| L04 Process Arrow | **선택** | 있으면 아이콘 제거, 없으면 우하단 아이콘 유지 |
| L05 Phased Columns | **선택** | 있으면 아이콘 제거, 없으면 우하단 아이콘 유지 |
| 기타 레이아웃 | 선택 | — |
...
- **우하단 아이콘 규격**: left=10.5", top=6.2", w=0.60", h=0.60" — 흐름도/도식도 없을 때 반드시 추가
  - 아이콘은 슬라이드 주제/내용에 맞는 PNG 선택 (icons/ 폴더에서 가장 적합한 것)
  - 흐름도 추가 시 아이콘 제거, 흐름도 제거 시 아이콘 복원 (항상 둘 중 하나 존재)
